...
export default function StaffRegistrationCompletion() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await authService.completeStaffRegistration(token, username.trim(), password);
      
      // Update invitation status in admin's local storage if it exists
      try {
        const email = response.data.email;
        const saved = localStorage.getItem('portal_staff_invitations_cache');
        if (saved && email) {
          const invites = JSON.parse(saved);
          const updated = invites.map(inv => 
            inv.email === email ? { ...inv, status: 'COMPLETED' } : inv
          );
          localStorage.setItem('portal_staff_invitations_cache', JSON.stringify(updated));
        }
      } catch (e) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex items-center justify-center p-6 transition-colors duration-200">
        <div className="max-w-md w-full text-center space-y-6 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Account Setup Complete!</h2>
          <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
            Your NSBM staff portal account has been activated. You can now log in using your newly created credentials.
          </p>
          <div className="pt-2">
            <Link to="/login">
              <Button className="w-full" icon="login">Proceed to Login</Button>
            </Link>
          </div>
        </div>
      </div>
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="max-w-md w-full space-y-6">
          <div className="text-center space-y-2">
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Complete Staff Registration</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">Set up your username and password to activate your portal privileges.</p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!searchParams.get('token') && !token && (
                <Input
                  ...
                  placeholder="Enter or paste invitation token"
                  required
                />
              )}
              ...
              <div className="pt-2">
                <Button type="submit" loading={loading} className="w-full" icon="how_to_reg">
                  Activate Account
                </Button>
              </div>
            </form>
