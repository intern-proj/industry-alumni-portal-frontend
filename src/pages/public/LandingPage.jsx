                    ...
                    <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                      ...
                      <div className="absolute top-3 right-3 flex flex-col items-end gap-1.5">
                        ...
                        {event.status === 'ONGOING' && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-600/90 text-white shadow-md backdrop-blur-md border border-emerald-400/30 animate-pulse">
                      ...
                      <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex gap-3">
                        <Link to={`/events/${event.id}`} className="flex-1">
                          <button className="w-full py-2.5 px-3 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                            {event.status === 'COMPLETED' ? 'View Recap' : 'Details'}
                          </button>
                        </Link>
                        {event.status === 'COMPLETED' ? (
                          <Link to={`/events/${event.id}`} className="flex-1">
                            <button className="w-full py-2.5 px-3 text-xs font-semibold rounded-xl bg-purple-50 dark:bg-purple-950/60 border border-purple-200 dark:border-purple-800 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors shadow-sm flex items-center justify-center gap-1">
                              <span className="material-symbols-outlined text-[14px]">photo_library</span>
                              Gallery
                            </button>
                          </Link>
                        ) : event.status === 'CANCELLED' ? (
                          <button disabled className="flex-1 py-2.5 px-3 text-xs font-semibold rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 cursor-not-allowed">
                            Cancelled
                          </button>
                        ) : (
